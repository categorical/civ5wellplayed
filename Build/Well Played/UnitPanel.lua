...
function UpdateUnitPortrait( unit )
	...
	--*** Promotion List BEGIN ***
	local promotions={}
	for p in GameInfo.UnitPromotions() do
		if unit:IsHasPromotion(p.ID) then
			local name=Locale.ConvertTextKey(p.Description)
			local desc=Locale.ConvertTextKey(p.Help)
			table.insert(
				promotions,
				string.format(
					"[COLOR_YELLOW]%s[ENDCOLOR]: %s",
					name,desc))
		end
	end
	if next(promotions)~=nil then
		Controls.UnitIcon:SetToolTipString(table.concat(promotions,"[NEWLINE]"));
	end
	--*** Promotion List END ***
